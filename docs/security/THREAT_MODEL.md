...
Status: living threat model, last reviewed 2026-08-11. This is not a
...
| Stripe spoofing                      | Billing and paid monitoring are off by default; the work-in-progress routes use raw-body signatures, event idempotency, current-period checks, durable duplicate-Checkout guards, and server-derived price/entitlement                                                                               | Redacted test catalog verifier passed; rotate the CLI-exposed test key and complete application Checkout/webhook/entitlement plus deployment checks before enablement |
...
The current local tree passed 85 files/449 tests and its production artifact ran
60 browser checks: 58 passed and two mobile checks were intentionally skipped,
including 24 axe checks and the complete authenticated API review/delivery/
idempotency journey. A 26-route HTTP verifier covered status/content types, CSP,
no-sniff, frame denial, the conditional HSTS contract, secret markers, ops
privacy, and two unknown-capability 404/private-cache probes. A separate manual
curl exercise confirmed every automated source and manual evidence remained
**Coming soon**/`UNVERIFIED` and Reddit remained **Permission
required**/`LEGAL_REVIEW`. This is working-tree `LOCAL_PASS`; final release-SHA
CI and external verification are still required.
