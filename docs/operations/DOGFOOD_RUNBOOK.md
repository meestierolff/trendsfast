...
The required production-equivalent internal set is exactly Halio and
ShipToUsers, in that order. A product name alone is not an accurate URL or
...
3. Submit through `POST /v1/projects/{project_id}/next-move`, poll through the
   real status endpoint, and run live only after each enabled provider's
   setup/rights/read-back gate.
...
5. Perform founder evidence review and edits, approve, deliver, retrieve through
   the API, and export both redacted JSON and Markdown review bundles. Use
   `pnpm dogfood:export <scan-id> --include-private-costs` for each required
   private bundle. The default export remains public-safe and omits monetary
   fields, but it does not satisfy this private review gate.
6. Compare both outputs. Generic repeated topic/action/advice is a launch
...
8. Keep scans private unless each subject explicitly opts into a public case
   study.
9. Keep every bundle under ignored `.var/private/dogfood/` with mode `0600`.
   Record the exact bundle paths, then stop for external review. Do not deploy
...
current 2026-08-13 status is **not run** because hosted infrastructure,
provider/model read-backs, and credentials are unavailable. No Halio or
ShipToUsers bundle path exists yet, and external review has not been reached.
